...
class TestClickTrackerFormatCoverage:
    ...
    def test_click_tracker_format_count(self):
        """Verify exactly 9 formats have click_tracker."""
        count = sum(
            1
    ...
    def test_click_tracker_only_on_clickable_formats(self):
        """Verify click_tracker is only on formats with click_url or landing_url."""
        for fmt in STANDARD_FORMATS:
            asset_ids = [a.asset_id for a in (fmt.assets or [])]
            if "click_tracker" in asset_ids:
                has_click_url = "click_url" in asset_ids or "landing_url" in asset_ids
                assert has_click_url, (
                    f"{fmt.format_id.id} has click_tracker but no click_url/landing_url"
                )
    ...
    def test_click_tracker_not_on_non_clickable_formats(self):
        ...
        non_clickable_types = ["audio", "dooh", "video"]
        for fmt in STANDARD_FORMATS:
